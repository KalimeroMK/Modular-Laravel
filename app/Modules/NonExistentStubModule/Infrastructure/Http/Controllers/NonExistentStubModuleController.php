...
class NonExistentStubModuleController extends Controller
{
    ...
    public function index(GetAllNonExistentStubModuleAction $action): JsonResponse
    {
        ...
        $resourceCollection = NonExistentStubModuleResource::collection($nonexistentstubmodules->items());
        $data = $resourceCollection->response()->getData(true);

        return response()->json([
            'status' => 'success',
            'message' => 'NonExistentStubModules retrieved successfully',
            'data' => $data['data'] ?? [],
            'meta' => [
                'current_page' => $nonexistentstubmodules->currentPage(),
                'last_page' => $nonexistentstubmodules->lastPage(),
                'per_page' => $nonexistentstubmodules->perPage(),
                'total' => $nonexistentstubmodules->total(),
                'from' => $nonexistentstubmodules->firstItem(),
                'to' => $nonexistentstubmodules->lastItem(),
            ],
            'links' => [
                'first' => $nonexistentstubmodules->url(1),
                'last' => $nonexistentstubmodules->url($nonexistentstubmodules->lastPage()),
                'prev' => $nonexistentstubmodules->previousPageUrl(),
                'next' => $nonexistentstubmodules->nextPageUrl(),
            ],
        ]);
    }
    ...
    {
        $nonexistentstubmodule = $action->execute($id);

        return ApiResponse::success(new NonExistentStubModuleResource($nonexistentstubmodule), 'NonExistentStubModule retrieved successfully');
    }
    ...
    public function store(CreateNonExistentStubModuleRequest $request, CreateNonExistentStubModuleAction $action): JsonResponse
    {
        ...
        return ApiResponse::created(new NonExistentStubModuleResource($nonexistentstubmodule), 'NonExistentStubModule created successfully');
    }
    ...
    {
        $dto = NonExistentStubModuleDTO::fromRequest($request);
        $updatedNonExistentStubModule = $action->execute($id, $dto);

        return ApiResponse::success(new NonExistentStubModuleResource($updatedNonExistentStubModule), 'NonExistentStubModule updated successfully');
    }
